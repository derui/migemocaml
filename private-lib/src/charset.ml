...
module Utf8 : Converter = struct
  ...
  let raw_to_character_for_noascii string =
    ...
    let detect_utf8_char_size byte =
      ...
      match utf8_char_size with 0xC0 -> 2 | 0xE0 -> 3 | 0xF0 -> 4 | _ -> 1
    in
    let size = detect_utf8_char_size Char.(code @@ string.[0]) in
    ...
    let rec combine_string_to_character string size offset ch =
      ...
      else if (Char.code @@ string.[offset]) land 0xc0 <> 0x80 then None
      else
        let shifted = Int64.shift_left ch 6 and masked = Int64.of_int @@ ((Char.code @@ string.[offset]) land 0x3f) in
        combine_string_to_character string size (succ offset) Int64.(add shifted masked)
    in
    if size < 2 then None
    else
      let ch = Int64.of_int @@ ((Char.code @@ string.[0]) land (0xff lsr size)) in
      match combine_string_to_character string size 1 ch with None -> None | Some c -> Some (c, size)

  let raw_to_character string =
    if String.length string = 0 then None
    else
      match raw_to_character_for_noascii string with
      | None           -> Some (Int64.of_int @@ Char.code @@ string.[0], 1)
      | Some (c, size) -> Some (c, size)

  let character_to_raw ch =
    (* TODO: very rebundand code... *)
    (* RFC-3629 says 5 and 6 string representasion of UTF-8 is obsolete, so this algorithm support utf-8 partially now. *)
