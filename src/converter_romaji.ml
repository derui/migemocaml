(** Romaji module provides functions to convert romaji to other character in japanese. But it does not provides function
    to convert other character to romaji, it is overspec. *)
...
module Core = struct
  ...
  let is_same_roma_letter_appear string =
    let open String in
    length string >= 2 && get string 0 = get string 1 && (not @@ contains non_xtu @@ get string 0)
  ...
  let convert ~string tree =
    ...
    let xtu_cache =
      match Dict_tree.forward_match ~query:normalized_xtu tree with
      | None | Some ([], _) -> None
      | Some (word :: _, _) -> Some (word, 1)
    and xn_cache =
      match Dict_tree.forward_match ~query:normalized_xn tree with
      | None | Some ([], _) -> None
      | Some (word :: _, _) -> Some (word, 1)
    in
    let rec loop string =
      if String.length string = 0 then ()
        ...
        let char, skip =
          if is_same_roma_letter_appear string then
            match xtu_cache with None -> convert_character string tree | Some (char, skip) -> (char, skip)
          else convert_character string tree
        in
        let char, skip =
          if char = converted_n && skip = 1 then
            match xn_cache with None -> (char, skip) | Some (word, skip) -> (word, skip)
          else (char, skip)
        in
