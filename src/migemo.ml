...
(** Make migemo object *)
let make ?romaji_to_hira ?hira_to_kata ?han_to_zen ?spec ~dict () =
  {
    dict;
    romaji_to_hira;
    hira_to_kata;
...
(** split query to segments. Normally upper case alphabets is splitter of segment.
    ...
    ["ab";"cde"] query="ABcDe" -> ["ab";"c";"de"] *)
let split_segments query =
  let is_uppercase_connected buf =
    if String.length buf < 2 then false else Util.(is_uppercase @@ buf.[0]) && Util.(is_uppercase @@ buf.[1])
  in
  let detect_splitter buf = if is_uppercase_connected buf then Util.is_lowercase else Util.is_uppercase in
  let rec inner_split_segments buf splitter accum segments =
    if String.length buf = 0 then
      match accum with [] -> List.rev segments | _ -> List.rev (String.concat "" (List.rev accum) :: segments)
    else
      let module C = Charset.Utf8 in
      match C.raw_to_character buf with
      | None              ->
          Printf.fprintf stderr "Invalid utf-8 sequence: %s" buf;
          segments
      | Some (code, skip) ->
          ...
          if skip = 1 then
            ...
            else inner_split_segments buf' splitter (ch_seq :: accum) segments
          else inner_split_segments buf' splitter (ch_seq :: accum) segments
  in
...
let add_word_list_to_gen query tree gen =
  ...
  | Some node ->
      D.traverse
        ~f:(fun node ->
          gen := List.fold_left (fun gen word -> Regexp_gen.add_word ~word gen) !gen node.D.Attr.word_list)
        node

let with_dict ~f dict = match dict with None -> () | Some dict -> f dict
...
let query_a_word word t =
  ...
  ( match t.han_to_zen with
  | None            -> ()
  | Some han_to_zen ->
      let converted = Converter_character.convert ~string:lower han_to_zen in
      gen := Regexp_gen.add_word ~word:converted !gen );
  (* Sequence to add words: original -> words what querying with original -> hiragana -> words what querying with
     hiragana -> katakana -> words what querying with katakana *)
  ...
  let regexps =
    List.map
      (fun word ->
        ...
        Regexp_gen.generate t.spec gen)
      segments
  in
